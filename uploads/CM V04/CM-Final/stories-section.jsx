...
function StoryCard({ s, w=300, ratio='3/4', i=0, big=false, hideHeadline=false, subline=false, cta='solid', reflect=false, reflectBleed=0 }) {
  ...
  const nameSize = big ? 23 : (hideHeadline ? 22 : 17);
  const card = (
    <a href={SS_BASE + s.slug} target="_blank" rel="noopener noreferrer"
      ...
      {/* delta badge */}
      <span style={{ position:'absolute', top:14, left:14, display:'inline-flex', alignItems:'center', gap:6, padding:'6px 12px', borderRadius:999,
        background:'var(--cm-badge-bg, rgba(8,12,22,0.5))', backdropFilter:'blur(8px)', border:'1px solid var(--cm-badge-bd, rgba(255,255,255,0.24))', font:`700 ${big?13:12}px/1 ${F.sans}`, letterSpacing:'.01em', color:'var(--cm-badge-fg, #fff)', boxShadow:'0 6px 18px rgba(8,12,22,0.35)' }}>
        {s.delta}
      </span>
      ...
      <div style={{ position:'absolute', left:0, right:0, bottom:0, padding: big?'24px':'18px' }}>
        <div style={{ marginBottom: subline ? 3 : (big?12 : (hideHeadline?6:9)) }}>
          <span style={{ font:`700 ${nameSize}px/1.2 ${F.sans}`, color:'#fff' }}>{s.name}</span>
        </div>
        {subline && <div style={{ display:'flex', alignItems:'center', gap:7, marginBottom: 3 }}>
          <span style={{ fontSize:15, lineHeight:1 }}>{s.flag}</span>
          {FLAG_COUNTRY[s.flag] && <span style={{ font:`500 13px/1 ${F.sans}`, color:'rgba(255,255,255,0.78)' }}>{FLAG_COUNTRY[s.flag]}</span>}
        </div>}
...
function WallGhost({ head, cta='solid' }) {
  ...
  const paused = useSSDrift(railRef, { axis:'x', speed:0.85 });
  const loop = [...STORIES, ...STORIES];
  ...
  return (
    <section data-screen-label="Stories · Aurora" style={{ position:'relative', width:PW, height:800, overflow:'hidden', background:SS.bg1, fontFamily:F.sans }}>
      ...
      <div aria-hidden style={{ position:'absolute', inset:0, display:'flex', flexDirection:'column', justifyContent:'center', gap:6, opacity:0.46, pointerEvents:'none',
        WebkitMaskImage:WALL_MASK_Y, maskImage:WALL_MASK_Y }}>
        {rows.map((faces,ri)=><FaceRow key={ri} faces={faces} dir={ri%2?'right':'left'} speed={150+ri*14} />)}
      </div>
      ...
      <div style={{ position:'relative', zIndex:2, height:'100%', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'flex-start', paddingTop:58 }}>
        <div className="gm-rise" style={{ position:'relative', marginBottom:30, padding:'0 64px' }}>
        ...
        <div className="gm-rise" style={{ animationDelay:'90ms', position:'relative', width:'100%' }}
          onMouseEnter={()=>paused.current=true} onMouseLeave={()=>paused.current=false}>
          <div ref={railRef} style={{ display:'flex', gap:14, overflowX:'auto', padding:'8px 64px 0', scrollbarWidth:'none',
            ...
            {loop.map((s,i)=><StoryCard key={i} s={s} i={i} w={300} ratio="3/4" hideHeadline subline cta={cta} reflect reflectBleed={14} />)}
          </div>
